...
const empty: Partial<Category> = { name: "", description: "", sort_order: 0 };
  ...
  return (
    <div>
      <div className="mb-4 flex items-center gap-3">
        <button onClick={() => { setError(""); setEditing({ ...empty }); }} className="btn-primary text-sm">
          + New Category
        </button>
      ...
      <div className="card overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {!loading && rows.length === 0 && <tr><td colSpan={4} className="p-6 text-center text-slate-400">No categories.</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/40 p-4" onClick={() => setEditing(null)}>
          <div className="card mt-12 w-full max-w-md space-y-3 p-6" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white">{editing.id ? "Edit Category" : "New Category"}</h2>
            <div>
              <label className="text-xs text-slate-500">Name *</label>
              <input className={input} value={editing.name ?? ""} onChange={(e) => setEditing({ ...editing, name: e.target.value })} />
            </div>
            <div>
              <label className="text-xs text-slate-500">Slug (auto if blank)</label>
