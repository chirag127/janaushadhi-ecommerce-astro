  ...
  return (
    <div>
              ...
              rows.map((o) => (
                ...
                >
                  ...
                  <td className="p-3 capitalize text-slate-500">
                    {addr(o)?._payment_method === "cod" ? "COD" : o.payment_status}
                  </td>
                  ...
                  <td className="p-3">
                    <select
                      ...
                      className="rounded border border-slate-300 px-2 py-1 text-xs capitalize dark:border-slate-700 dark:bg-slate-900"
                    >
                      {STATUSES.map((s) => (
                        <option key={s} value={s}>
                          {s}
      ...
      {open && (
        ...
        >
          <div
            className="card mt-8 w-full max-w-lg space-y-4 p-6"
          ...
          >
            ...
            {addr(open) && (
              <div className="rounded-lg bg-slate-50 p-3 text-sm dark:bg-slate-800/50">
                ...
                <p className="text-slate-500">
                  {addr(open)?.line1}, {addr(open)?.city}, {addr(open)?.state}{" "}
                  {addr(open)?.pincode}
                </p>
                <p className="text-slate-500">📞 {addr(open)?.phone}</p>
                <p className="mt-1 text-slate-500">
                  Payment:{" "}
                  {addr(open)?._payment_method === "cod"
                    ? "Cash on Delivery"
                    : "Razorpay (online)"}
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <select
                ...
                className="rounded-lg border border-slate-300 px-3 py-2 text-sm capitalize dark:border-slate-700 dark:bg-slate-900"
              >
                {STATUSES.map((s) => (
                  <option key={s} value={s}>
                    {s}
